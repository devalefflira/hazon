...
export default function Orcamentos({ onVoltarParaHome, usuarioLogado, usuarioLogadoId }: OrcamentosProps) {
  ...
  const [precoFinalManual, setPrecoFinalManual] = useState<number>(0);
  ...
  // Modal de Detalhes da Aba Concluídos
  ...
  // Calcula Preço Unitário Final conforme Modo de Desconto
  const calcularPrecoUnitarioFinal = () => {
    ...
    if (modoDesconto === 'SEM_DESCONTO') {
      ...
      const desc = (precoTabela * (percentualDesconto || 0)) / 100;
      return Math.max(0, precoTabela - desc);
    } else {
      return precoFinalManual >= 0 ? precoFinalManual : precoTabela;
    }
  };

  const precoUnitarioCalculado = calcularPrecoUnitarioFinal();
  ...
  const handleAdicionarItem = (e: React.FormEvent) => {
    ...
    const subtotal = precoUnitarioCalculado * quantidade;
    ...
    const novoItem = {
      ...
      preco_venda_tabela: Number(produtoSelecionado.pvenda || 0),
      percentual_desconto: modoDesconto === 'PERCENTUAL' ? percentualDesconto : 0,
      preco_final_unitario: precoUnitarioCalculado,
      valor_total_item: subtotal
    };

    setItensAdicionados((prev) => [...prev, novoItem]);

    // Reseta form do produto
    setProdutoSelecionado(null);
    setTermoBuscaProduto('');
    setQuantidade(1);
    setModoDesconto('SEM_DESCONTO');
    setPercentualDesconto(0);
    setPrecoFinalManual(0);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 font-sans flex flex-col items-center select-none">
      ...
      {/* FORMULÁRIO / TELA DE NOVO ORÇAMENTO */}
      {emEdicao && (
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4 select-none">
          <div className="w-full max-w-2xl bg-white rounded-3xl p-6 shadow-2xl flex flex-col gap-4 max-h-[90vh]">
            ...
            <div className="overflow-y-auto flex flex-col gap-4 pr-1 flex-1">
              ...
              {/* SELEÇÃO E INCLUSÃO DE PRODUTOS */}
              <form onSubmit={handleAdicionarItem} className="bg-emerald-50/50 border border-emerald-200 p-3.5 rounded-2xl flex flex-col gap-3">
                <span className="text-[10px] font-black text-emerald-800 uppercase">Adicionar Produto no Orçamento</span>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-2 text-xs font-bold">
                    <button
                      type="button"
                      onClick={() => setModoDesconto('SEM_DESCONTO')}
                ...
                {modoDesconto === 'MANUAL' && (
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) => setPrecoFinalManual(Number(e.target.value))}
                      className="w-full h-9 text-xs bg-white border border-gray-200 px-3 rounded-xl font-bold text-gray-800"
                    />
                  </div>
                )}
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-black text-gray-400 uppercase">Itens do Orçamento ({itensAdicionados.length})</span>
                  ...
                  <div key={item.temp_id} className="p-3 bg-gray-50 border border-gray-200 rounded-xl flex justify-between items-center text-xs font-bold">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 mt-0.5">
                        Qtd: {item.quantidade} {item.unidade_medida} x {(item.preco_final_unitario || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                      </p>
                    </div>
      ...
      {/* MODAL DE RESUMO (EXCLUSIVO PARA CONCLUÍDOS) */}
      {orcamentoDetalhe && (
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4 select-none">
                  ...
                  <div key={item.id} className="p-3 bg-gray-50 border border-gray-200 rounded-xl flex justify-between items-center text-xs font-bold">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400">
                        {item.quantidade} {item.unidade_medida || 'UN'} x {(item.preco_final_unitario || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                      </p>
                    </div>
                    <span className="font-mono text-[#09797a]">
                      {(item.valor_total_item || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
