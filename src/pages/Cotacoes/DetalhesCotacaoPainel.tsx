...
export function DetalhesCotacaoPainel({ cotacaoId, onVoltar, onSucesso }: DetalhesCotacaoPainelProps) {
    ...
    async function carregarDadosAuditoria() {
      try {
        ...
        const { data: mestre, error: errMestre } = await supabase
          ...
          .select(`id, status, created_at, usuarios:comprador_id ( nome )`)
          .eq('id', cotacaoId)
          .single();
        ...
        const { data: itensVinculados, error: errItens } = await supabase
          ...
          .select(`notas_falta ( id, produtos ( id, descricao, codigo_barras ) )`)
        ...
        (itensVinculados || []).forEach((iv: any) => {
          ...
          if (produto) {
            const propostasDoItem: PropostaItem[] = respostas
              ...
              .map((r: any) => {
                const fornRaw = r?.vinculo?.fornecedores;
                const fornObj = Array.isArray(fornRaw) ? fornRaw[0] : fornRaw;
                return {
                  resposta_item_id: String(r?.id || ''),
                  fornecedor: String(fornObj?.nome_fantasia || 'Fornecedor'),
        ...
        console.error('Erro ao processar mapa de auditoria:', err);
      } finally {
        // CORREÇÃO: Alterado de finaly para finally
        setLoading(false);
      }
    }
  ...
  const handleConcluirRodada = async () => {
    ...
    if (itensGanhadores.length === 0) {
      alert('Nenhuma proposta recebida para concluir esta cotação.');
      return;
    }
    ...
    try {
      ...
      alert('🏆 Rodada concluída com sucesso!');
      onSucesso();
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-100 flex justify-center items-start p-4 font-sans">
      <div className="w-full max-w-95 bg-white rounded-4xl shadow-xl px-5 py-6 flex flex-col min-h-[calc(100vh-32px)]">
        <div className="flex items-center gap-3 w-full mb-5 border-b border-gray-100 pb-4">
          <button onClick={onVoltar} className="p-2 hover:bg-gray-50 rounded-full text-[#09797a] font-bold text-xl">←</button>
          <div>
            <h1 className="text-[#09797a] font-black text-base uppercase leading-tight">Auditoria</h1>
            <p className="text-[10px] text-gray-400 font-mono">Ref: #{cotacaoId.substring(0, 8).toUpperCase()}</p>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto max-h-[calc(100vh-230px)] pb-4 flex flex-col gap-4">
          {produtosComPropostas.map((prod) => (
            <div key={prod.id} className="border border-gray-200 rounded-3xl p-4 bg-gray-50/40 flex flex-col gap-2.5 shadow-sm">
              <div>
                <h4 className="text-xs font-black text-gray-800 uppercase">{prod.descricao}</h4>
              ...
              <div className="flex flex-col gap-2">
                {prod.propostas.length === 0 ? (
                  <p className="text-[11px] text-gray-400 italic py-1">Sem propostas para este item...</p>
                ) : (
                  prod.propostas.map((prop, idx) => (
                    <div key={idx} className={`p-3 rounded-2xl border flex flex-col gap-1 ${idx === 0 ? 'bg-emerald-50/60 border-emerald-200' : 'bg-white border-gray-100'}`}>
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-gray-700 truncate max-w-[60%]">{idx === 0 ? '🥇 ' : ''}{prop.fornecedor}</span>
                        <span className={`text-xs font-black ${idx === 0 ? 'text-emerald-700' : 'text-gray-800'}`}>R$ {prop.preco.toFixed(2)}</span>
                      </div>
        ...
        {cotacaoMestre?.status !== 'Concluída' && (
          <div className="pt-4 border-t border-gray-100 mt-auto">
            <button
              ...
              className="w-full bg-[#09797a] text-white py-4 rounded-3xl text-xs font-bold flex justify-center items-center disabled:opacity-50"
            >
