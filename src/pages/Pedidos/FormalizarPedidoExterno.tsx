// Arquivo: src/pages/Pedidos/FormalizarPedidoExterno.tsx
    ...
    async function carregarDadosPedido() {
      try {
        ...
        const { data: mestre, error: errMestre } = await supabase
          .from('pedidos_mestre')
          .select(`id, codigo_customizado, status, fornecedores:fornecedor_id ( nome_fantasia )`)
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex justify-center items-center p-4 font-sans">
        <div className="w-full max-w-md bg-white rounded-4xl shadow-xl p-8 text-center border border-gray-100">
          ...
          <h2 className="text-teal-700 font-black text-xl uppercase tracking-wide">Pedido Formalizado!</h2>
          <p className="text-xs text-gray-400 font-medium mt-2 leading-relaxed">
            A confirmação da ordem foi registrada com sucesso no Hazon ERP.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 flex justify-center items-start p-4 font-sans selection:bg-transparent">
      <div className="w-full max-w-md bg-white rounded-4xl shadow-xl px-5 py-6 flex flex-col border border-gray-100">
        ...
        <div className="mb-5 border-b border-gray-100 pb-4">
          <h1 className="text-teal-700 font-black text-base uppercase leading-tight">Confirmar Ordem de Compra</h1>
          <p className="text-[11px] text-orange-600 font-bold mt-0.5 font-mono">
            Ref: {pedidoMestre?.codigo_customizado} | {pedidoMestre?.fornecedores?.nome_fantasia || 'Parceiro Comercial'}
          </p>
        </div>
            ...
            {itens.map((item) => (
              <div key={item.id} className="p-3 bg-gray-50/60 border border-gray-200 rounded-2xl flex justify-between items-center gap-4">
                ...
                <div className="text-right flex flex-col items-end shrink-0">
                  <span className="text-xs font-black text-teal-700 bg-white border border-gray-100 px-3 py-1 rounded-xl">
        ...
        <div className="mt-auto pt-4 border-t border-gray-100 flex flex-col gap-4">
          ...
          <button
            ...
            className="w-full bg-teal-700 text-white py-4 rounded-3xl text-xs font-bold shadow-md disabled:opacity-50 active:scale-95 transition-all"
          >
