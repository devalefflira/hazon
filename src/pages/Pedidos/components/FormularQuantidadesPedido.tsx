...
interface FormularQuantidadesPedidoProps {
  pedido: PedidoMestreDTO;
  onVoltar: () => void;
}

export function FormularQuantidadesPedido({ pedido, onVoltar }: FormularQuantidadesPedidoProps) {
  ...
  const handleEnviarVendedor = async (e: React.FormEvent) => {
    ...
    const algumaQuantidadeInvalida = itens.some(item => item.quantidade_solicitada <= 0);
    ...
    <div className="min-h-screen bg-gray-100 flex justify-center items-start p-4 font-sans selection:bg-transparent">
      <form onSubmit={handleEnviarVendedor} className="w-full max-w-95 bg-white rounded-4xl shadow-xl px-5 py-6 flex flex-col min-h-[calc(100vh-32px)] relative">
        ...
        <div className="flex items-center gap-3 w-full mb-5 border-b border-gray-100 pb-4">
          ...
          <div>
            <h1 className="text-[#09797a] font-black text-base uppercase leading-tight">Estipular Volumes</h1>
            <p className="text-[10px] text-gray-400 font-mono font-bold">Ordem: {pedido.codigo_customizado}</p>
          </div>
        </div>

        {/* LISTAGEM DE PRODUTOS PARA DIGITAÇÃO */}
        <div className="flex-1 overflow-y-auto max-h-[calc(100vh-270px)] pb-4 flex flex-col gap-3">
          <div className="p-3 bg-teal-50/40 border border-teal-100 rounded-2xl mb-1">
            <span className="text-[10px] font-black text-[#09797a] uppercase tracking-wider block">Fornecedor Ganhador</span>
            ...
            <div key={item.id} className="border border-gray-200 rounded-3xl p-4 bg-gray-50/40 flex flex-col gap-3 shadow-sm">
              <div>
                ...
                <div className="flex justify-between items-center mt-1">
                  ...
                  <span className="text-[10px] text-gray-500 font-bold bg-white px-2 py-0.5 border border-gray-100 rounded-lg">Preço: R$ {item.preco_unitario.toFixed(2)}</span>
                </div>
              </div>

              <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-2xl px-4 py-2.5 focus-within:border-[#09797a]">
                <input
                  type="number"
                  step="any"
                  required
                  ...
                  placeholder="Defina a Qtd"
                  className="w-full text-xs font-black text-gray-800 focus:outline-none bg-transparent"
                />
                <span className="text-[10px] font-black text-gray-400 uppercase">{item.produto_unidade_medida}</span>
              </div>
            </div>
          ))}
        </div>

        {/* BOTÃO MESTRE E SUBMETER */}
        <div className="pt-4 border-t border-gray-100 mt-auto flex flex-col gap-3 bg-white w-full">
          <div className="flex justify-between items-center px-1">
            <span className="text-[10px] font-black text-gray-400 uppercase">Total Estimado</span>
          ...
          <button
            type="submit"
            disabled={salvando}
            className="w-full bg-[#09797a] text-white py-4 rounded-3xl text-xs font-bold shadow-md active:scale-95 transition-all flex justify-center items-center"
          >
